...
#Query 14: Find the total quantity of products ordered by each user.
SELECT user.user_id, user.username, (SELECT SUM(quantity) FROM order_item WHERE order_item.user_id = user.user_id) AS total_ordered
FROM user
LIMIT 0, 1000;

#Query 15: Find all products with a price higher than the average price of products in the "Classic Black Pen" category
SELECT *
FROM product
WHERE price > (SELECT AVG(price) FROM product WHERE pc_id = (SELECT pc_id FROM product_category WHERE name = 'Classic Black Pen'));
...
SELECT user.user_id, SUM(quantity) AS total_ordered
FROM order_item
GROUP BY user_id
